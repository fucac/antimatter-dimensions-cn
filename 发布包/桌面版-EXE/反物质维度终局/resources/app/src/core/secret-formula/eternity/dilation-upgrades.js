...
export const dilationUpgrades = {
  ...
  tachyonGain: rebuyable({
    ...
    description: () => {
      if (Pelle.isDoomed && !PelleDestructionUpgrade.x3TPUpgrade.canBeApplied) return `将获得的超光速粒子数量乘以 ${formatInt(1)}`;
      if (Enslaved.isRunning) return `将获得的超光速粒子数量
      乘以 ${Math.pow(3, Enslaved.tachyonNerf).toFixed(2)}`;
      return "将获得的超光速粒子数量 tripled";
    },
    effect: bought => {
  ...
  doubleGalaxies: {
    ...
    description: () => (Alpha.isDestroyed
      ? `获得两倍超光速星系`
      : `获得两倍超光速星系，上限为 ${formatInt(500)} 基础星系`),
    effect: 2
  },
  tdMultReplicanti: {
    id: 5,
    ...
    description: () => {
      ...
      return `时间维度受复制器乘数影响 ${formatPow(multiplier, 1, 3)}，超过 ${formatX(DC.E9000)} 时效果降低`;
    },
    effect: () => {
  ...
  ndMultDT: {
    id: 6,
    cost: 5e7,
    description: "基于膨胀时间的反物质维度乘数，不受时间 dilation 影响",
  ...
  timeStudySplit: {
    id: 8,
    cost: 1e10,
    description: "你可以从维度分裂购买所有三条时间研究路径"
  },
  dilationPenalty: {
    id: 9,
    cost: 1e11,
    description: () => `降低 dilation 惩罚（降低后为 ${formatPow(1.05, 2, 2)}）`,
    effect: 1.05,
  },
  ttGenerator: {
    id: 10,
    cost: 1e15,
    description: "基于超光速粒子生成时间定理",
  ...
  galaxyMultiplier: rebuyable({
    ...
    pelleOnly: true,
    description: "将获得的超光速星系相乘，在 TG 翻倍升级后生效",
