...
export class EternityChallengeState extends GameMechanicState {
  ...
  start(auto) {
    ...
    if (this.id === 12 && ImaginaryUpgrade(24).isLockingMechanics && Ra.isRunning && maxInversion) {
      if (!auto) ImaginaryUpgrade(24).tryShowWarningModal("进入永恒挑战 12");
      return false;
    }
    if (this.id === 7 && ImaginaryUpgrade(15).isLockingMechanics && TimeDimension(1).amount.gt(0)) {
      if (!auto) ImaginaryUpgrade(15).tryShowWarningModal("进入永恒挑战 7");
      return false;
    }
  ...
  fail(auto = false) {
    ...
    if (auto) {
      if (this.id === 4) {
        reason = restriction => `自动永恒挑战完成完成了` +
        `永恒挑战 ${this.id}，并使下一阶段` +
        `要求拥有比您当前更少的无限（${quantifyInt("Infinity", restriction)} ` +
        `或更少）`;
      } else if (this.id === 12) {
        reason = restriction => `自动永恒挑战完成完成了` +
        `永恒挑战 ${this.id}，并使下一阶段` +
        `要求在其中花费比您已花费更少的时间（${quantify("in-game second", restriction, 0, 1)} ` +
        `或更少）`;
      }
    } else if (this.id === 4) {
      reason = restriction => `您未能通过永恒挑战 ${this.id}，因为您` +
      `拥有超过 ${quantifyInt("Infinity", restriction)}`;
    } else if (this.id === 12) {
      reason = restriction => `您未能通过永恒挑战 ${this.id}，因为您` +
      `在其中花费了超过 ${quantify("in-game second", restriction, 0, 1)}`;
    }
