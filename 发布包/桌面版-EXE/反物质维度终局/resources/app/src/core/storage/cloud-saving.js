...
export const Cloud = {
  ...
  async login() {
    ...
    try {
      ...
      if (player.options.hideGoogleName) GameUI.notify.success(`已成功登录 Google 账号`);
      else GameUI.notify.success(`已成功以 ${this.user.displayName} 登录`);
      if (ShopPurchaseData.isIAPEnabled) Speedrun.setSTDUse(true);
    } catch (e) {
      GameUI.notify.error("Google 账号登录失败");
    }
  },
  ...
  save() {
    ...
    if (STEAM) {
      GameUI.notify.info(`游戏已保存（存档 ${slot + 1}）到云端，用户 ${this.user.displayName}`);
      return;
    }

    if (player.options.hideGoogleName) GameUI.notify.info(`游戏已保存（存档 ${slot + 1}）到云端`);
    else GameUI.notify.info(`游戏已保存（存档 ${slot + 1}）到云端，用户 ${this.user.displayName}`);
  },
  ...
  async loadCheck() {
    ...
    if (save === null) {
      if (player.options.hideGoogleName) GameUI.notify.info(`当前 Google 账号没有云端存档`);
      else GameUI.notify.info(`用户 ${this.user.displayName} 没有云端存档`);
    } else {
      ...
      const overwriteLocalSave = () => {
        ...
        if (STEAM) {
          GameUI.notify.info(`云端存档已加载`);
          return;
        }

        if (player.options.hideGoogleName) GameUI.notify.info(`云端存档（存档 ${saveId + 1}）已加载`);
        else GameUI.notify.info(`云端存档（存档 ${saveId + 1}）已为用户 ${this.user.displayName} 加载`);
      };
