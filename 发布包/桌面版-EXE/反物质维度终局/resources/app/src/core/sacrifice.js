export class Sacrifice {
  ...
  static get disabledCondition() {
    if (NormalChallenge(10).isRunning) return "8th 维度已被禁用";
    if (EternityChallenge(3).isRunning) return "永恒挑战 3";
    if (DimBoost.purchasedBoosts.lt(5)) return `需要 ${formatInt(5)} 次维度提升`;
    if (AntimatterDimension(8).totalAmount.eq(0)) return "无 8th 反物质维度";
    if (this.nextBoost.lte(1)) return `${formatX(1)} 倍率`;
    if (Player.isInAntimatterChallenge) return "已达挑战目标";
    return "需要进行大坍缩";
  }
