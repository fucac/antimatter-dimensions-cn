...
export const shortcuts = [
  {
    ...
    function: () => {
      ...
      GameUI.notify.info(`时间研究重置现已 ${player.respec ? "已开启" : "已关闭"}`);
    },
    ...
    function: () => {
      ...
      GameUI.notify.info(`符文重置现已 ${player.reality.respec ? "已开启" : "已关闭"}`);
    },
    ...
    function: () => {
      ...
      GameUI.notify.info(`终局精通重置现已 ${player.endgame.respec ? "已开启" : "已关闭"}`);
    },
    visible: () => PlayerProgress.endgameUnlocked()
  }, {
    name: "切换天神快捷键",
    ...
    function: () => {
      ...
      GameUI.notify.info(`简单快捷键天神命令现已 ${player.options.simpleHotkeysCelestialMode ? "已开启" : "已关闭"}`);
    },
...
function toggleAutobuyer(buyer) {
  ...
  if (buyer.disabledByContinuum) {
    GameUI.notify.info("连续统已启用，你无法更改此自动购买器");
  } else if (buyer.isUnlocked || isSimpleTickspeed) {
    buyer.toggle();
    GameUI.notify.info(`${buyer.name}自动购买器已${(buyer.isActive) ? "开启" : "关闭"}`);
  }
  return false;
}
...
function toggleBuySingles(buyer) {
  if (buyer.disabledByContinuum) {
    ...
    buyer.toggleMode();
    const bulkName = (buyer.name === "Tickspeed" || buyer.hasUnlimitedBulk) ? "最大" : "10";
    GameUI.notify.info(`${buyer.name}自动购买器设置为购买${(buyer.mode === 1) ? "单个" : bulkName}`);
  }
  return false;
}
  ...
  if (Player.automatorUnlocked) {
    if (AutomatorBackend.isRunning) {
      ...
      if (AutomatorData.currentErrors().length === 0) {
        GameUI.notify.automator(`正在启动脚本 "${AutomatorBackend.scriptName}"`);
      } else {
        GameUI.notify.error(`无法启动脚本 "${AutomatorBackend.scriptName}"（有错误）`);
      }
      return;
    }
    const action = AutomatorBackend.isRunning ? "恢复" : "暂停";
    const linenum = AutomatorBackend.currentLineNumber;
    GameUI.notify.automator(`${action}脚本 "${AutomatorBackend.scriptName}" 第 ${linenum} 行`);
  }
}
