...
export const ENSLAVED_UNLOCKS = {
  FREE_TICKSPEED_SOFTCAP: {
    ...
    description: () => `将来自时间维度的 Tickspeed 升级软上限提高 ${formatInt(1e5)}`,
  },
  RUN: {
    id: 1,
...
export const Enslaved = {
  ...
  useStoredTime(autoRelease) {
    ...
    if (ImaginaryUpgrade(24).isLockingMechanics && Ra.isRunning && maxInversion) {
      if (!autoRelease) ImaginaryUpgrade(24).tryShowWarningModal("释放您的黑洞");
      return;
    }
  ...
  feelEternity() {
    if (this.feltEternity) {
      Modal.message.show(`您已经揭开了这道现实中的裂缝。本次永恒中的时间将以您的永恒次数为倍数增加，
        最高可达 ${formatX(1e66)}。`,
      { closeEvent: GAME_EVENT.REALITY_RESET_AFTER }, 1);
    } else {
      EnslavedProgress.feelEternity.giveProgress();
      this.feltEternity = true;
      Modal.message.show(`本次永恒中的时间将以您的永恒次数为倍数增加，
        最高可达 ${formatX(1e66)}。`, { closeEvent: GAME_EVENT.REALITY_RESET_AFTER }, 1);
    }
  },
  get feltEternity() {
...
class EnslavedProgressState extends BitUpgradeState {
  ...
  giveProgress() {
    ...
    if (this.hasHint && !this.hasProgress) {
      ...
      GameUI.notify.success("您发现了无名氏现实中的一道裂缝！", 10000);
    }
