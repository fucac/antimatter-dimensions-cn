...
export const Pelle = {
  ...
  initializeRun() {
    ...
    if (!Glyphs.unequipAll()) {
      Modal.hideAll();
      Modal.message.show(`毁灭您的现实将卸下您的符文。由于物品栏空间不足，
        您的一些符文无法卸下。`, 1);
      return;
    }
    ...
    if (Glyphs.freeInventorySpace < 5) {
      Modal.hideAll();
      Modal.message.show(`您必须有足够的未受保护空符文槽位来容纳
        ${formatInt(5)} 个额外符文，才能毁灭您的现实。`, 1);
      return;
    }
  ...
  getSpecialGlyphEffectDescription(type, count = 1, onlyReturnUseful = false) {
    switch (type) {
      case "infinity":
        return `无限点数获得 ${formatX(this.calculatePelleInfinity(count), 2)}（基于当前无限点数）`;
      case "time":
        return `永恒点数获得 ${formatX(this.calculatePelleTime(count), 2)}
          （基于当前永恒点数）`;
      case "replication":
        return `复制体速度 ${formatX(this.calculatePelleReplication(count), 2)}
        （基于${wordShift.wordCycle(PelleRifts.vacuum.name)}）`;
      case "dilation":
        return `膨胀时间获得 ${formatX(this.calculatePelleDilation(count), 2)}
          （基于超光速星系）`;
      case "power":
        return `星系强度提升 ${formatPercents(this.calculatePellePower(count) - 1)}`;
      case "companion":
        return `你感觉好了 ${formatPercents(0.34)}`;
  ...
  get endTabNames() {
    if (player.celestials.pelle.divinities >= 13) {
      return "吾辈 新神 仍 遍历 此 尘世 寻觅 微力 戏谑 混乱 之 狂笑 声 焉".split(" ");
    } else if (player.celestials.pelle.divinities > 0) {
      return "吾辈 再启 此行 重生 必至 奋勇 向前 Ω 之中 神 引领 之 途".split(" ");
    } else if (Effarig.isRunning) {
      return "恭贺 君 方才 突破 双重 天神 实相 Ω Ω 佩勒 亦 深感 惊叹".split(" ");
    } else if (Alpha.isDestroyed) {
      return "为何 仍 驻 此 间 万物 尽毁 无遗 唯存 Ω Ω 机丝 辰星".split(" ");
    } else if (Achievement(191).isUnlocked) {
      return "毁灭 已至 新章 既 已 启 Ω Ω Ω 我辈 终将 重逢".split(" ");
    } else {
      return "此 未 终结 吾辈 必 将 归来 Ω Ω Ω Ω 须臾 即 重逢".split(" ");
    }
  },
