...
export default {
  ...
  computed: {
    ...
    multiplierDisplay() {
      ...
      return `每次升级速度 ${formatX(tickmult, 2, 3)} 倍。`;
    },
    celestialTickspeedDisplay() {
      return `天神 Tickspeed：${format(this.celestialTickspeed, 2, 3)} / 秒`;
    },
    upgradeCount() {
      const purchased = this.purchasedCelestialTickspeed;
      return quantifyHybridLarge("已购升级", purchased);
    }
  },
  methods: {
    update() {
