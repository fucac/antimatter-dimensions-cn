...
export default {
  ...
  computed: {
    ...
    tooltipContents() {
      ...
      if (this.isCapped) return `无名阻止你购买超过 ${format(1)} 个时间维度`;
      if (this.isContinuumActive) return "连续统产出你所有的时间维度";
    ...
    formattedEPCost() {
      ...
      return `${this.showCostTitle ? "成本：" : ""}${format(this.cost, 2)} 永恒点数`;
    },
    continuumString() {
    ...
    timeEstimate() {
      ...
      return time.gt(0) ? `还需 ${TimeSpan.fromSeconds(time).toStringShort()} 获得足够时间定理` : "";
    }
  },
  watch: {
