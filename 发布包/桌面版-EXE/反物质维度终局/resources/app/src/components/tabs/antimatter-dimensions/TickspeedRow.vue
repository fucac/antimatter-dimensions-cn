...
export default {
  ...
  computed: {
    ...
    multiplierDisplay() {
      if (InfinityChallenge(3).isRunning) return `所有反物质维度 ×${formatX(this.galaxyCount.times(0.005).add(1.05), 3, 3)}`;
      const tickmult = this.mult;
      return `每次升级速度 ${formatX(tickmult.reciprocal(), 2, 3)} 倍。`;
    },
    tickspeedDisplay() {
      return `Tickspeed：${format(this.tickspeed, 2, 3)} / 秒`;
    },
    continuumString() {
    ...
    upgradeCount() {
      ...
      if (!this.freeTickspeed) return quantifyHybridLarge("已购升级", purchased);
      if (purchased.eq(0) || this.isContinuumActive) return `${formatHybridLarge(this.freeTickspeed, 3)} 免费升级`;
      return `${formatHybridLarge(purchased, 3)} 已购 + ${formatHybridLarge(this.freeTickspeed, 3)} 免费`;
    }
  },
  methods: {
    update() {
    ...
    <div class="tickspeed-buttons">
      ...
      >
        <span v-if="isContinuumActive">
          Tickspeed 连续统：{{ continuumString }}
        </span>
        <span v-else-if="isEC9">
          Tickspeed 无法购买（永恒挑战 9）
        </span>
        <span v-else>
          Tickspeed 成本：{{ format(cost) }}
        </span>
        <div
          v-if="hasTutorial"
