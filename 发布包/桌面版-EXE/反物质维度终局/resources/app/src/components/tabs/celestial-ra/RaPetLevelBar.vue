...
export default {
  ...
  computed: {
    ...
    nextScalingUpgrade() {
      ...
      switch (this.pet.name) {
        case "Teresa":
          return "可额外充能 1 个无限升级";
        case "Effarig":
          return `在炼金术中解锁「${effarigAlchemyResource.name}」资源——${effarigAlchemyResource.description}`;
        case "Enslaved":
          return `储存的游戏时间 ×${formatX(20)}，并可多储存 1 小时真实时间`;
        case "V":
          return "可额外购买 1 个三元研究";
        default:
          return "false";
      }
    },
