...
export default {
  ...
  computed: {
    ...
    nextAtDisplay() {
      ...
      if (first) return `您将于 ${TimeSpan.fromMilliseconds(this.timeToNextAscension)} 后抵达首次飞升。`;
      return next === undefined
        ? "已无更多飞升可供抵达！"
        : `您将于 ${TimeSpan.fromMilliseconds(this.timeToNextAscension)} 后抵达下一次飞升。`;
    },
    nextHintDisplay() {
      return `下一次飞升为「${this.nextAscension?.config.name}」。`;
    },
    runButtonOuterClass() {
      return {
    ...
    symbol() {
      return this.isRunning
        ? `退出过载，获 ${formatInt(this.pending)} ${this.currEnergyName}（下次于 ${format(this.nextAt, 2)} 永恒点数）`
        : "进入过载";
    },
    currEnergy() {
    ...
    currEnergyName() {
      if (this.currentLevel === 4) return "时之能量";
      if (this.currentLevel === 3) return "复数能量";
...
<template>
  <div>
    <div class="l-endgame-milestone-grid">
      <div>
        <span class="c-ascension-description-text">
          您拥有 {{ format(divineEnergy, 2, 2) }} 神圣能量。+{{ format(divineEnergyPerSecond, 2, 2) }}/s
        </span>
      </div>
      <div>
        <span class="c-ascension-description-text">
          抵达下一次飞升之时，将随您神圣能量之多寡而缩减。
        </span>
      </div>
      <div>
        <span class="c-ascension-description-text">
          您当前飞升层数为 {{ formatInt(ascension) }}。
        </span>
      </div>
      <div
        v-for="row in rows"
    ...
    <div v-if="highestUnlockedLevel >= 1">
      <span class="c-ascension-description-text">您拥有 {{ infiniteEnergy }} 无限能量。</span>
    </div>
    <div v-if="highestUnlockedLevel >= 2">
      <span class="c-ascension-description-text">您拥有 {{ eternalEnergy }} 永恒能量。</span>
    </div>
    <div v-if="highestUnlockedLevel >= 3">
      <span class="c-ascension-description-text">您拥有 {{ complexEnergy }} 复数能量。</span>
    </div>
    <div v-if="highestUnlockedLevel >= 4">
      <span class="c-ascension-description-text">您拥有 {{ temporalEnergy }} 时之能量。</span>
    </div>
  </div>
</template>
