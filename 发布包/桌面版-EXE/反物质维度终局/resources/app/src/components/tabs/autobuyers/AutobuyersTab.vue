...
<template>
  <div class="l-autobuyers-tab">
    ...
    <div v-if="hasSeenGamespeedAlteringEffects">
      自动购买器的间隔与基于时间的设置始终以<b>现实时间</b>为准，因此
      <br>
      不受任何影响游戏本身运行速度的因素所干扰。
      <br>
      <br>
    </div>
    <div v-if="!hasInfinity">
      用于升级自动购买器的挑战，将于抵达无限之时解锁。
    </div>
    <b>未显示批量数的自动购买器，默认即为无限批量。</b>
    <b>
      当反物质维度自动购买器的间隔低于 {{ formatInt(100) }} 毫秒时，可升级其批量数。
    </b>
    <b v-if="hasInstant">间隔为「瞬时」的自动购买器将于每一游戏刻触发一次（{{ gameTickLength }}）。</b>
