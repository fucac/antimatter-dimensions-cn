...
<template>
  <div class="l-large-hadron-collider-tab">
    ...
    <div v-if="highestAntimatter.gt(10)">
      <span class="c-void-antimatter-amount">[你在虚空内达到的最高反物质为 {{ format(highestAntimatter, 2, 1) }}。]</span>
      <br>
      <span class="c-null">[你拥有 {{ formatNullAmount(nullMatter) }} 虚空物质。+{{ formatNullAmount(nullPerSecond) }}/秒]</span>
    </div>
    <div v-if="nullified">
      <span class="c-null">[你拥有 {{ format(nullParticles, 2, 2) }} 虚空粒子。+{{ format(nullParticlesPerSecond, 2, 2) }}/秒]</span>
    </div>
