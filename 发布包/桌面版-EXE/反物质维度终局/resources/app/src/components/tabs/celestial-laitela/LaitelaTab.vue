...
<template>
  <div class="l-laitela-celestial-tab">
    ...
    <div class="o-laitela-matter-amount">
      ...
      <span v-if="!isDMCapped">（平均：{{ format(darkMatterGain, 2, 2) }}/s）</span>
    </div>
    <div class="o-laitela-matter-amount">
      您历来最多之暗物质为
      <span :style="styleObject">{{ format(maxDarkMatter, 2) }}</span><span v-if="!isDoomed">，
        赋予连续统额外 {{ formatContinuumPercentage() }} 次购买</span>。
    </div>
    <div class="o-laitela-matter-amount">
      暗物质维度不受存储现实时间之影响。
    </div>
    ...
    >
      暗物质于超过 {{ format(softcap1, 2) }} 后将受软上限所制。
    </div>
    <div
      v-if="maxDarkMatter.gte(softcap2)"
      class="o-laitela-matter-amount"
    >
      暗物质于超过 {{ format(softcap2, 2) }} 后将受更甚之软上限所制。
    </div>
    <div
      v-if="endgameUnlocked"
      class="o-laitela-matter-amount"
    >
      暗物质于 <span v-if="isUncapped">受严苛软上限所制</span><span v-if="!isUncapped">受硬上限所制</span>，上限为
