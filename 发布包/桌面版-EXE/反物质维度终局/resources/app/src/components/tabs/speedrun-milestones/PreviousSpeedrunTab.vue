...
<template>
  <div class="c-previous-runs">
    <b>你已在此前完成了 {{ quantify("次速通", numRuns, 0, 0, x => x) }}。</b>
    <b>此前速通的统计数据如下，将鼠标悬停在图标上可查看详情。</b>
    <b>点击放大镜可将某次速通的里程碑与本次速通进行对比。</b>
    <b>{{ spectateText }}</b>
    <br>
    <div
    ...
    >
      <PrimaryButton
        ...
        @click="changePage(-1)"
      />
      显示第 {{ 10 * runPage + 1 }} 至 {{ 10 * (runPage + 1) }} 次速通（共 {{ highestIndex }} 次）
      <PrimaryButton
    ...
    <div class="c-legend">
        <div class="c-legend-cell">
          <span class="o-box l-milestone-none" /> 本次未达成
        </div>
        <div class="c-legend-cell">
          <span class="o-box l-milestone-slow" /> 慢于对比
        </div>
        <div class="c-legend-cell">
          <span class="o-box l-milestone-fast" /> 快于对比
        </div>
        <div class="c-legend-cell">
          <span class="o-box l-milestone-fastest" /> 快于最佳
        </div>
      </div>
