...
export default {
  ...
  computed: {
    ...
    boostText() {
      ...
      if (this.achMultToCDS) boostList.push(`天神维度：${achCDEffect}`);
      if (this.achMultToVDS) boostList.push(`神圣维度：${achVDEffect}`);

      if (this.achMultToTP) boostList.push(`超光速粒子：${achTPEffect}`);
      if (this.achMultToBH) boostList.push(`黑洞之力：${achievementPower}`);
      if (this.achMultToTT) boostList.push(`时间定理产出：${achievementPower}`);
      if (this.achMultToEnt) boostList.push(`熵之生成：${achEnEffect}`);
    ...
    megaBoostText() {
      ...
      if (this.achMultToCDS) powersList.push(`天神维度：${achCDPow}`);
      if (this.achMultToVDS) powersList.push(`神圣维度：${achVDPow}`);

      if (this.achMultToTP) powersList.push(`超光速粒子：${achTPPow}`);
      if (this.achMultToBH) powersList.push(`黑洞之力：${achievementPowers}`);
      if (this.achMultToTT) powersList.push(`时间定理产出：${achievementPowers}`);
      if (this.achMultToEnt) powersList.push(`熵之生成：${achEnPow}`);
  ...
  <div class="l-achievements-tab">
    ...
    <div class="c-achievements-tab__header c-achievements-tab__header--multipliers">
      <span v-if="isDoomed && !isDestroyed">
        所有成就倍数已被禁用<SwapAchievementImagesButton />
      </span>
      <span v-else>
        成就提供倍数于<SwapAchievementImagesButton />
        <div v-html="boostText" />
      </span>
      <span v-if="showPowers">
        成就亦提供次方于<SwapAchievementImagesButton />
        <div v-html="megaBoostText" />
      </span>
    </div>
    <div class="c-achievements-tab__header">
      带有 <i class="fas fa-star" /> 图标的成就还会给予额外奖励。
    </div>
    <div
      v-if="showAutoAchieve"
    ...
    >
      <div v-if="achCountdown.gt(0)">
        将在
        {{ timeDisplayNoDecimals(achCountdown) }}<span v-if="!isAutoAchieveActive">（开启自动后）</span> 后自动获得下一个缺失的成就。
        （从左到右，从上到下）
      </div>
      <div v-else-if="missingAchievements !== 0">
        一旦启用自动成就，即自动获得下一个缺失的成就。
        （从左到右，从上到下）
      </div>
      <div v-if="totalCountdown.gt(0)">
        若自动成就<span v-if="isAutoAchieveActive">保持开启</span><span v-else>被开启</span>，你将在 {{ timeDisplayNoDecimals(totalCountdown) }} 后重新获得所有剩余成就。
      </div>
      <br>
    </div>
