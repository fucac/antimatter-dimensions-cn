...
export default {
  ...
  computed: {
    formatMachinesGained() {
      if (this.machinesGained.gt(0)) return `现实机器获得：${format(this.machinesGained, 2)}`;
      return "未获得现实机器";
    },
    formatMachineStats() {
      ...
      if (this.machinesGained.gt(0) && this.machinesGained.lt(100)) {
        return `（下次于 ${format(this.nextMachineEP, 2)} 永恒点数）`;
      }
      ...
      if (this.newIMCap.neq(0) && this.newDMCap.eq(0)) {
        return `（虚幻机器上限：${formatMachines(0, this.newIMCap, 0)})`;
      }
      if (this.newDMCap.neq(0)) {
        return `（εM 上限：${formatMachines(0, 0, this.newDMCap)})`;
      }
      if (this.machinesGained.lt(Number.MAX_VALUE)) {
        return `（${format(this.machinesGained.divide(this.realityTime), 2, 2)} 现实机器/分）`;
      }
      return "";
    },
    formatGlyphLevel() {
      if (this.glyphLevel.gte(10000)) return `符文等级：${formatHybridLarge(this.glyphLevel, 3)}`;
      return `符文等级：${formatHybridLarge(this.glyphLevel, 3)}（距下次 ${this.nextGlyphPercent}）`;
    },
    showsRate() {
    ...
    warpMessage() {
      return false ? "诅咒你的现实" : "进入 Pelle 之域";
    },
    classObject() {
      return {
  ...
  methods: {
    ...
    handleClick() {
      if (this.readyToWarp) {
        Modal.message.show(`此功能将于 v2.0 开放。感谢游玩 Antimatter Dimensions: Endgame！`, {}, 3);
        //requestRealityWarp();
      }
    ...
    formatThresholdText(condition, threshold, resourceName) {
      if (condition) return "";
      return `（${format(threshold, 2, 2)} ${resourceName} 可提升）`;
    },
    ...
    >
      <div class="l-reality-button__contents">
        ...
        <template v-else-if="hasRealityStudy">
          <div>获得 {{ format("1e4000") }} 永恒点数以解锁新现实</div>
        </template>
        <template v-else>
          <div>于永恒标签页购买该研究以解锁新现实</div>
        </template>
        <div
        ...
        >
          ...
          <div v-if="shardsGained.neq(0)">
            ...
            <br>
            峰值：{{ format(bestShardRate, 2) }}/分，于 {{ format(bestShardRateVal, 2) }} 遗迹碎片
          </div>
          <div
