...
const pluralDatabase = new Map([
  ...
  ["Null Matter", "虚空物质"],
  ["Relic Shard", "遗迹碎片"],
  ["Tachyon Particle", "超光速粒子"],
  ["Tachyon Particles", "超光速粒子"],
  ["total Time Theorem", "时间定理"],
  ["Total Time Theorems", "时间定理"],
  [" 个天神点", " 个天神点"],
  [" 个毁灭微粒", " 个毁灭微粒"],
  ["无限点数", "无限点数"],
  ["天神点数", "天神点数"],
  ...
  ["复兴点数", "复兴点数"],
  ["个复兴点数", "个复兴点数"],
  // English→Chinese resource names (used by pluralize() in header/tab templates)
  ["Infinity Point", "无限点数"],
  ["Celestial Infinity Point", "天神无限点数"],
  ["Eternity Point", "永恒点数"],
  ["Celestial Eternity Point", "天神永恒点数"],
  ["Reality Machine", "现实机器"],
  ["Reality Shard", "现实碎片"],
  ["Divine Star", "神圣星辰"],
  ["Remnant", "遗物"],
  ["Time Theorem", "时间定理"],
  ["Endgame Skill Point", "终局天赋点"],
  ["Hadron", "强子"],
  ["Dark Hadron", "暗强子"],
  ["Exotic Hadron", "奇异强子"],
  ["Galaxy", "星系"],
...
window.pluralize = function pluralize(word, amount, plural) {
  ...
  // English→Chinese override (also applied when singular)
  const existingPlural = plural ?? pluralDatabase.get(word);
  // If the override is Chinese (or exists and amount is plural), use it
  if (existingPlural !== undefined &&
      (/[\u4e00-\u9fff]/u.test(existingPlural) || !isSingular(amount))) {
    return existingPlural;
  }

  if (isSingular(amount)) return word;

  // Chinese (CJK) characters do not pluralize — return as-is
  if (/[\u4e00-\u9fff]/u.test(word)) {
    pluralDatabase.set(word, word);
    return word;
  }

  const newWord = existingPlural ?? generatePlural(word);
