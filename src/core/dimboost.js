...
export class DimBoost {
  ...
  static get unlockedByBoost() {
    ...
    if (!allNDUnlocked && boosts.lt(DimBoost.maxDimensionsUnlockable - 4)) {
      newUnlock = `解锁第 ${formatInt(boosts.add(5))} 反物质维度`;
    } else if (boosts.eq(4) && !NormalChallenge(10).isRunning && !EternityChallenge(3).isRunning) {
      newUnlock = "解锁维度献祭";
    }

    const multText = `为反物质维度提供 ${formatX(DimBoost.power, 2, 1)} 倍率`;
    let dimensionRange = `（第 1 维度）`;
    if (boosts.gt(0)) dimensionRange = `（第 1-${Decimal.min(boosts.add(1), 8)} 维度）`;
    if (boosts.gte(DimBoost.maxDimensionsUnlockable - 1)) dimensionRange = `（所有反物质维度）`;
    const powText = Ascensions.dbA.isUnlocked ? `，并为所有维度提供 +${formatPow(DimBoost.exponentialPower, 2, 3)} 幂` : "";

    let boostEffects;
    if (NormalChallenge(8).isRunning) boostEffects = newUnlock;
    else if (newUnlock === "") boostEffects = `${multText}${dimensionRange}${powText}`;
    else boostEffects = `${newUnlock}；${multText}${dimensionRange}${powText}`;

    if (boostEffects === "") return "维度提升当前无效果";
