...
export const AD = {
  total: {
    name: dim => {
      if (dim) return `AD ${dim} 乘数`;
      if (NormalChallenge(12).isRunning) {
        if (MultiplierTabHelper.actualNC12Production().eq(0)) return "所有维度的基础AD产量";
        return `${MultiplierTabHelper.isNC12ProducingEven() ? "偶数" : "奇数"}维度的基础AD产量`;
      }
      return "基础AD产量";
    },
    displayOverride: dim => {
  ...
  highestDim: {
    ...
    displayOverride: () => {
      ...
      return `反物质维度 ${dim}，${format(AntimatterDimension(dim).totalAmount, 2)}`;
    },
    multValue: () => {
      const dim = EternityChallenge(7).isRunning ? 7 : MultiplierTabHelper.activeDimCount("AD");
  ...
  dimboost: {
    name: dim => (dim ? `AD ${dim} 的维度提升` : "维度提升"),
    multValue: dim => (dim
  ...
  effectNC: {
    name: dim => (dim ? `普通挑战效果（AD ${dim}）` : "普通挑战效果"),
  ...
  nerfIC: {
    name: dim => (dim ? `无限挑战削弱（AD ${dim}）` : "无限挑战削弱"),
    multValue: dim => {
      let dimMults = Array.repeat(DC.D1, 9);
