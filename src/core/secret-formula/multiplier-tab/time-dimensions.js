...
export const TD = {
  total: {
    name: dim => {
      if (dim) return `TD ${dim} 乘数`;
      if (EternityChallenge(7).isRunning) return "ID8 产出";
      return "时间碎片产出";
    },
  ...
  eternityUpgrade: {
    name: dim => (dim ? `其他永恒升级 (TD ${dim})` : "其他永恒升级"),
    multValue: dim => {
      const allMult = DC.D1.timesEffectsOf(
  ...
  tickspeed: {
    ...
    displayOverride: () => {
      ...
      return `${format(tickRate, 2, 2)}/秒 on ${formatInt(activeDims)} ${dimString}
