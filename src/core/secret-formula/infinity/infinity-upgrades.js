...
export const infinityUpgrades = {
  ...
  dim18mult: {
    ...
    description: "第一与第八维度依据无限次数，增益倍率",
  ...
  dim27mult: {
    ...
    description: "第二与第七维度依据无限次数，增益倍率",
    effect: () => dimInfinityMult(),
    formatEffect: value => formatX(value, 1, 1),
    charged: {
  ...
  dim36mult: {
    ...
    description: "第三与第六维度依据无限次数，增益倍率",
    effect: () => dimInfinityMult(),
    formatEffect: value => formatX(value, 1, 1),
    charged: {
  ...
  dim45mult: {
    ...
    description: "第四与第五维度依据无限次数，增益倍率",
    effect: () => dimInfinityMult(),
    formatEffect: value => formatX(value, 1, 1),
    charged: {
  ...
  buy10Mult: {
    id: "dimMult",
    cost: () => Math.pow(1, Alpha.isRunning ? AlphaUnlocks.infinity.effects.nerf.effectOrDefault(1) : 1),
    description: () => `提升购买 ${formatInt(10)} 个反物质维度之倍率`,
    effect: () => 1.1,
  ...
  thisInfinityTimeMult: {
    ...
    description: "反物质维度于当前无限之境历练，增益倍率",
  ...
  unspentIPMult: {
    ...
    description: "依未耗之无限点数，增益第一反物质维度之倍率",
    effect: () => {
  ...
  ipGen: {
    ...
    description: () => `被动生成无限点数，速率为最快无限之 ${formatInt(10)} 分之一`,
    ...
    formatEffect: value => {
      if (Teresa.isRunning || V.isRunning) return "此现实中已禁用";
      if (Pelle.isDoomed && !PelleDestructionUpgrade.x2IPUpgrade.canBeApplied) return "已禁用";
      if (Alpha.isRunning && player.records.bestInfinity.realTime >= 999999999999) return "过于迟缓，无法生成";
      if (player.records.bestInfinity.time.gte(999999999999)) return "过于迟缓，无法生成";
      if (Alpha.isRunning) return `${format(value, 2)} 每 ${TimeSpan.fromMilliseconds(Time.bestInfinityRealTime.totalMilliseconds.times(10)).toStringShort()}`;
      return `${format(value, 2)} 每 ${TimeSpan.fromMilliseconds(Time.bestInfinity.totalMilliseconds.times(10)).toStringShort()}`;
    },
    charged: {
      description: () =>
  ...
  skipReset1: {
    ...
    description: () =>
      `每次重置起始赐予 ${formatInt(1)} 维度提升，自动解锁第五反物质维度`,
  },
  skipReset2: {
    id: "skipReset2",
    ...
    description: () =>
      `每次重置起始赐予 ${formatInt(2)} 维度提升，自动解锁第六反物质维度`,
  },
  skipReset3: {
    id: "skipReset3",
    ...
    description: () =>
      `每次重置起始赐予 ${formatInt(3)} 维度提升，自动解锁第七反物质维度`,
  },
  skipResetGalaxy: {
    id: "skipResetGalaxy",
    ...
    description: () =>
      `每次重置起始赐予 ${formatInt(4)} 维度提升，自动解锁第八反物质维度；并赐予一反物质星系`,
  },
  ipOffline: {
    id: "ipOffline",
    ...
    description: () => (player.options.offlineProgress
      ? `仅离线时，获取 ${formatPercents(0.5)} 的最快无限点数/分钟（不使用最大购买）`
      : "此升级可提供离线无限点数生成，但当前已禁用离线进度功能"),
  ...
  ipMult: {
    ...
    description: () => Ascensions.ipA.isUnlocked ? `无限点数指数增加 +${formatPow(0.01, 2, 2)}` : `所有来源的无限点数乘以 ${formatX(2)}`,
