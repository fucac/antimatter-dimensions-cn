...
export const Speedrun = {
  ...
  unlock() {
    ...
    Modal.message.show(`您已解锁速通模式！这允许您开启一个带有少许更改的新存档文件，如果您想尽快通关游戏，这会很有帮助。在选项标签页的"保存"下方，现在可以找到开始速通存档的选项。选择开始速通存档后，将弹出一个包含更详细信息的模态框。`, {}, 3);
...
class SpeedrunMilestone extends GameMechanicState {
  ...
  complete() {
    ...
    player.speedrun.records[this.config.id] = Math.round(player.records.realTimePlayed.toNumber());
