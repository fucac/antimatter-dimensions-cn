...
export const Glyphs = {
  ...
  equip(glyph, targetSlot) {
    ...
    if (glyph.type !== "companion") {
      if (RealityUpgrade(9).isLockingMechanics) {
        if (this.activeWithoutCompanion.length > 0) {
          RealityUpgrade(9).tryShowWarningModal("装备另一个非伙伴符文");
          return;
        }
        if (glyph.level.lt(3)) {
          RealityUpgrade(9).tryShowWarningModal(`装备一个等级低于 ${formatInt(3)} 的符文`);
          return;
        }
      }
    ...
    if (specialGlyphEquippedAfterChange > maxSpecialGlyph) {
      Modal.message.show(`您只能装备 ${formatInt(maxSpecialGlyph)} 个 ${glyph.type.capitalize()} 符文！`,
  ...
  unequipAll(forceToUnprotected = false) {
    ...
    if (stillEquipped && !fastReality) {
      const target = player.options.respecIntoProtected ? "受保护槽位" : "主物品栏";
      const hasOther = this.findFreeIndex(!player.options.respecIntoProtected) !== -1;
      setTimeout(() => Modal.message.show(`${quantifyInt("Glyph", stillEquipped)} 由于空间不足无法卸下。
        请在您的${target}中腾出一些空间${hasOther ? "，或切换您卸下符文的目标位置" : ""}，
        以便卸下${stillEquipped === 1 ? "它" : "它们"}。`, { closeEvent: GAME_EVENT.GLYPHS_CHANGED }),
      50);
    }
  ...
  giveCursedGlyph() {
    if (GameCache.glyphInventorySpace.value === 0) {
      Modal.message.show("没有可用的物品栏空间；献祭一些符文以腾出空间。",
        { closeEvent: GAME_EVENT.GLYPHS_CHANGED });
      return;
    }
    ...
    if (cursedCount >= maxCount) {
      GameUI.notify.error(`您不需要超过 ${format(maxCount)} 个诅咒符文！`);
    } else {
      this.addToInventory(GlyphGenerator.cursedGlyph());
      GameUI.notify.error("已创建一个诅咒符文");
    }
  }
};
...
export function respecGlyphs() {
  if (!Glyphs.unequipAll()) {
    Modal.message.show("由于物品栏空间不足，您的一些符文无法卸下。",
      { closeEvent: GAME_EVENT.GLYPHS_CHANGED });
  }
