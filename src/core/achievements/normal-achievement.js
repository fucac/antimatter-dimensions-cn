...
class AchievementState extends GameMechanicState {
  ...
  unlock(auto) {
    ...
    if (this.id === 55 && !PlayerProgress.realityUnlocked()) {
      Modal.message.show(`由于您在不到一分钟内完成了一次无限，屏幕上的界面发生了变化。
        维度不再消失，而是保留在原处，大紧缩按钮会出现在它们上方。
        这纯粹是视觉效果，用于防止闪烁。`, {}, 3);
    }
    ...
    if (auto) {
      GameUI.notify.reality(`已自动解锁：${this.name}`);
    } else {
    ...
    if (player.speedrun.isActive && !player.speedrun.achievementTimes[this.id]) {
      ...
      player.speedrun.achievementTimes[this.id] = Math.floor(player.records.realTimePlayed.toNumber());
    }
