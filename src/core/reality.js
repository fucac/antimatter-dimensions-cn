...
export function requestManualReality() {
  ...
  if (GameCache.glyphInventorySpace.value === 0) {
    Modal.message.show("没有可用的物品栏空间；通过按住 Shift 点击符文将其丢弃以腾出空间。",
...
function giveRealityRewards(realityProps) {
  ...
  if (Teresa.isRunning) {
    ...
    const modalText = `您已完成特蕾莎的现实！${isHigher
      ? `由于您获得了更多反物质，您的符文献祭倍数
      从 ${format(current, 2, 2)} 提升到了 ${format(newMultiplier, 2, 2)}`
      : `您在本次运行中没有获得更多反物质，因此来自特蕾莎的符文献祭倍数
      没有提升`}.`;
    ...
    glyphsToProcess,
    {
      ...
      asyncEntry: doneSoFar => {
        ...
        ui.$viewModel.modal.progressBar = {
          label: "正在模拟放大的现实",
          info: () => `游戏正在计算您从重复刚刚完成的现实 ${formatInt(glyphsToProcess)} 次中将获得的所有资源。当剩余符文超过
            ${formatInt(glyphsToSample)} 个时，按下"快速符文"将通过自动献祭所有剩余可获得的符文来加速计算。按下"跳过符文"将忽略所有与符文相关的资源，
            并在给予所有其他资源后停止模拟。
            ${Ra.unlocks.unlockGlyphAlchemy.canBeApplied ? `按下任一加速按钮都不会更新符文炼金术中的任何资源。` : ""}`,
          progressName: "现实次数",
          current: doneSoFar,
          max: glyphsToProcess,
          startTime: Date.now(),
