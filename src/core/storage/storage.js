...
export const GameStorage = {
  ...
  checkPlayerObject(save) {
    ...
    if (save.money === undefined && save.antimatter === undefined) return "存档没有反物质属性";
    ...
    return `${quantify("NaN player property", invalidProps.length)} 已找到：
      ${invalidProps.join(", ")}`;
  },
  ...
  save(silent = true, manual = false) {
    ...
    if (!silent) GameUI.notify.info("游戏已保存");
  },
  ...
  export() {
    ...
    GameUI.notify.info("已将当前存档文件导出到剪贴板");
  },
...
(${this.exportDateString}).txt`, save);
    GameUI.notify.info("已成功将当前存档文件下载到您的计算机");
  },
...
(${this.exportDateString}).txt`, GameSaveSerializer.serialize(backupData));
    GameUI.notify.info("已成功将存档文件备份下载到您的计算机");
  },
  ...
  importBackupsFromFile(importText) {
    ...
    this.resetBackupTimer();
    GameUI.notify.info("已成功从文件导入存档文件备份");
  },
