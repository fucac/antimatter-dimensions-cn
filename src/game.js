...
export function addEternityTime(time, realTime, ep, eternities) {
  ...
  if (player.challenge.eternity.current) {
    ...
    const challText = player.dilation.active ? "膨胀永恒挑战" : "永恒挑战";
    challenge = `${challText} ${currEC} (${formatInt(ec.completions)}/${formatInt(ec.maxCompletions)})`;
  } else if (player.dilation.active) challenge = "膨胀时间";
      ...
      ticks,
      {
        ...
        asyncEntry: doneSoFar => {
          ...
          ui.$viewModel.modal.progressBar = {
            label: "离线进度模拟",
            info: () => `游戏正在以较低精度运行，用于快速计算你在离线期间获得的资源。
              技术详情请查看「新手指引」中的「离线进度」条目。如果你等不及要尽快回到游戏，
              可以点击「加速」按钮以一半的 tick 数模拟剩余时间（最低保留 ${formatInt(500)} 个 tick）。
              「跳过」按钮则会用 ${formatInt(10)} 个 tick 消耗完所有剩余离线时间。`,
            progressName: "Tick数",
            current: doneSoFar,
            max: ticks,
            startTime: Date.now(),
