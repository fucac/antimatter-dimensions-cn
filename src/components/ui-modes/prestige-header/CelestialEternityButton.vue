...
<template>
  ...
  >
    ...
    <template v-else-if="type === 0">
      神圣的时间线正等待着……我需要成为天神永恒之身
    </template>

    <!-- Normal -->
    <template v-else-if="type === 1">
      <span v-if="showCEPRate">天神永恒获得 </span>
      <span v-else>天神永恒获得 </span>
      <span :style="amountStyle">{{ format(gainedCEP, 2) }}</span>
      <span v-if="showCEPRate"> 天神永恒点数</span>
      <span v-else> {{ pluralize("个天神永恒", gainedCEP) }}点</span>
      <br>
      <template v-if="showCEPRate">
        当前：{{ format(currentCEPRate, 2, 2) }} 天神永恒点数/分
        <br>
        峰值：{{ format(peakCEPRate, 2, 2) }} 天神永恒点数/分
        <br>
        于 {{ format(peakCEPRateVal, 2, 2) }} 天神永恒点数
      </template>
    </template>
    ...
    <template v-else-if="type === 2">
      <template>
        天神永恒获得 <span :style="amountStyle">{{ format(gainedCEP, 2, 2) }}</span> 天神永恒点数
      </template>
      <br>
      有人在等你。不要让他们久等。
    </template>
