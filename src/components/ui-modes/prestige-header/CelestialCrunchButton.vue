...
<template>
  ...
  >
    ...
    <template v-else>
      ...
      <b>
        <span v-if="showCIPRate">天神紧缩获得 </span>
        <span v-else>天神大紧缩获得 </span>
        <span :style="amountStyle">{{ format(gainedCIP, 2) }}</span>
        <span v-if="showCIPRate"> 天神点数</span>
        <span v-else> {{ pluralize("个天神", gainedCIP) }}无限点数</span>
      </b>
      <template v-if="showCIPRate">
        <br>
        当前：{{ format(currentCIPRate, 2) }} 天神点数/分
        <br>
        峰值：{{ format(peakCIPRate, 2) }} 天神点数/分
        <br>
        于 {{ format(peakCIPRateVal, 2) }} 天神点数
      </template>
      <div v-else />
    </template>
  </button>
