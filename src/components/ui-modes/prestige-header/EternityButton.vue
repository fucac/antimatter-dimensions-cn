  ...
  >
    ...
    <template v-else-if="type === 1">
      ...
      <template v-if="showEPRate">
        ...
        <br>
        发生在 {{ format(peakEPRateVal, 2, 2) }} 永恒点数
      </template>
    </template>
    ...
    <template v-else-if="type === 3">
      永恒获得 <span :style="tachyonAmountStyle">{{ format(gainedTachyons, 2, 1) }}</span>
    ...
    <template v-else-if="type === 4 || type === 5">
      <template v-if="type === 4">
        永恒获得 <span :style="amountStyle">{{ format(gainedEP, 2, 2) }}</span> 永恒点数
      </template>
      <template v-else>
        永恒获得 <span :style="tachyonAmountStyle">{{ format(gainedTachyons, 2, 1) }}</span> 超光速粒子
      </template>
      <br>
      在点击我之前，你应该先探索一下新内容！
    </template>
    ...
    <template v-else-if="type === 6">
      ...
      <template v-else>
        <br>
        永恒后将获得 {{ quantifyInt("次完成", gainedCompletions) }}
