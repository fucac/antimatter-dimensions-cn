...
export default {
  ...
  computed: {
    ...
    nextAtDisplay() {
      ...
      if (first) return `首个扩展包在反物质达到 ${format(next)} 时解锁。`;
      if (alpha && !Pelle.isDoomed) return `反物质达到 ${format(next)} 即可解锁新扩展包。`;
      if (alpha && Pelle.isDoomed) return `毁灭现实期间无法购买下一个扩展包！`;
      return (next === undefined || alphaLock)
        ? "所有扩展包已解锁"
        : `下一个扩展包在反物质达到 ${format(next)} 时解锁。`;
    },
    classObject() {
      return {
