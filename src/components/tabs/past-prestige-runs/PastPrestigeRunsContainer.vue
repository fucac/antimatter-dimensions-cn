...
export default {
  ...
  computed: {
    ...
    points() {
      ...
      const currencyMap = { RM: "现实机器", EP: "永恒点数", IP: "无限点数" };
      if (rawText === "RM" && this.hasIM) return "虚幻机器上限";
      return currencyMap[rawText] || rawText;
    },
    condition() {
  ...
  methods: {
    ...
    infoArray(run, index) {
      ...
      else name = `${formatInt(index + 1)} 轮前`;
    ...
    infoCol() {
      const cells = ["运行", this.hasRealTime ? "游戏时间" : "运行时间"];
    ...
    prestigeCurrencyGain(run) {
      if (this.hasIM && this.layer.name === "Reality") return `${format(run[7], 2)} 虚幻机器`;
    ...
    prestigeCurrencyRate(run) {
      if (this.hasIM && this.layer.name === "Reality") return "不适用";
    ...
    challengeText(run) {
      ...
      const rawText = run[4];
      return rawText === "The Nameless Ones" ? "无名氏" : rawText;
    },
    toggleShown() {
    ...
    <div v-show="shown">
      ...
      >
        ...
        >
          ...
          <i v-else>
            你尚未进行 {{ formatInt(index + 1) }}
            次 {{ index === 0 ? singular : plural }}。
          </i>
        </span>
        <span
          v-else
