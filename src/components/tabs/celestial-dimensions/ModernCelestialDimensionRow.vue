...
export default {
  ...
  computed: {
    ...
    name() {
      return `第 ${CelestialDimension(this.tier).shortDisplayName} 天神维度`;
    },
    costDisplay() {
      if (this.isUnlocked || this.shiftDown) {
        if (this.isCapped) return "已上限";
        return this.showCostTitle ? `成本：${format(this.cost)} 天神点` : `${format(this.cost)} 天神点`;
      }
      ...
      return `达到 ${format(CelestialDimension(this.tier).cpRequirement)} 天神点`;
    },
    hasLongText() {
      return this.costDisplay.length > 20;
    },
    capTooltip() {
      if (this.isCapped) return `上限于 ${format(this.capCP)} 天神点时达成`;
