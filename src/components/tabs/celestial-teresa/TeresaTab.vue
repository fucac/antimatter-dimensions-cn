...
export default {
  ...
  computed: {
    ...
    autoText() {
      return this.autoPour ? "自动开启" : "自动关闭";
    },
    pourText() {
    ...
    lastMachinesString() {
      return this.lastMachines.gte(DC.E20000)
        ? `${quantify("对偶机", this.lastMachines.dividedBy(DC.E20000), 2)}`
    ...
    chargeDisplay() {
      return `充能升级：${this.chargeView ? "开启" : "关闭"}`;
    },
...
<template>
  <div class="l-teresa-celestial-tab">
    ...
    <div v-if="chargeUnlocked">
      ...
      被充能的复兴升级其效果会被改变。
      <br>
      按住 Shift 键可显示充能后的复兴升级。你可在终局时自由洗点重选。
    </div>
    <div>
    ...
    <div class="l-mechanics-container">
      ...
      >
        <div class="c-teresa-unlock c-teresa-run-button">
          ...
          <br><br>
          <div>
            此现实可重复挑战，在其中获得的反物质越多，重复奖励越强。
            <br><br>
            <span v-if="showRunReward">
              你在特蕾莎现实中的反物质记录是 {{ format(bestAM, 2) }}，
              通过获得 {{ lastMachinesString }} 达成。
              <br><br>
              使用的符文套装：
              <GlyphSetPreview
                text="特蕾莎最佳符文套装"
        ...
        >
          特蕾莎现实奖励：符文献祭威力 {{ formatX(runReward, 2, 2) }}
        </div>
        <div
          v-if="hasEPGen"
          class="c-teresa-unlock"
        >
          <span :class="{ 'o-pelle-disabled': isEPGenDoomed }">
            本次现实期间，每秒获得你峰值永恒点数的 {{ formatPercents(0.01) }} / 分钟。
          </span>
        </div>
      </div>
