...
export default {
  ...
  computed: {
    ...
    playTooltip() {
      if (this.isPaused) return "继续运行自动机";
    ...
    statusText() {
      ...
      if (this.isPaused) return `已暂停："${this.statusName}"（从第 ${lineNum} 行继续）`;
