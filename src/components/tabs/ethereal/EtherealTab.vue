...
export default {
  ...
  computed: {
    ...
    extraPowerDisplay() {
      return `当星系之力超过 ${format(DC.NUMMAX, 2, 2)} 时,其亦以此为据。`;
    },
    etherealClassObject() {
      return {
    ...
    starPowerReqText() {
      return `达到 ${format(DC.NUMMAX, 2, 2)} 星辉乘积以解锁星力。`;
    },
    starPowerDisplay() {
    ...
    nextGenerationText() {
      if (this.allGenerationsUnlocked) return `所有星力奖励均已解锁`;
      return `当星力达到 ${format(this.nextGeneration, 2, 2)} 时,你将获得新的星力奖励`;
    },
    starTexts() {
      let arr = [];
      let starName = ["红", "橙", "黄", "绿", "蓝", "紫", "白", "黑", "灰"];
      for (let t = 0; t < 9; t++) {
        if (Ethereal.starGeneration(t).neq(0)) {
          arr.push(`星力当前每秒生成 ${formatDecimalPercents(this.starGen[t], 2)} 待定的${starName[t]}星`);
        }
      }
      return arr;
    }
...
<template>
  <div class="l-ethereal-tab">
    ...
    >
      <div v-if="!canExtend">
        <span class="c-normal-ethereal-text">达到 {{ format(1e25, 2, 2) }} 以太能量以拓展以太。</span>
      </div>
      <div v-if="canExtend">
        <button
    ...
    >
      <div>
        <span class="c-stellar-glow">你的星辉乘积为 </span>
    ...
    >
      <div>
        ...
        <span class="c-stellar-glow"> 星力。</span>
        <span class="c-cooler-stellar-glow">+{{ format(starPowerPerSecond, 3, 3) }}/s</span>
      </div>
      <div>
        <span class="c-stellar-glow">你的星力当前以 </span>
        <span class="c-cooler-stellar-glow">{{ formatX(starBoost, 3, 3) }}</span><span class="c-stellar-glow"> 的倍率提升所有星宿类型的获取。</span>
      </div>
      <br>
      <span
