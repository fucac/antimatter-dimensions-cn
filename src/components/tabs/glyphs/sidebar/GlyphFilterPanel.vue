...
export default {
  ...
  computed: {
    ...
    questionmarkTooltip() {
      return `所有符文选择都会被赋予分数，并与基于所选模式的阈值进行比较。
        分数最高的符文会被选中，但如果低于阈值仍会被献祭。
        （点击查看更多详情）`;
    },
    autoRealityTooltip() {
      return `若自动现实已开启，则忽略所有其他设置，若没有即将出现的符文会被保留，则立即进行现实`;
    },
    unlockedModes() {
...
<template>
  <div class="l-glyph-sacrifice-options c-glyph-sacrifice-options l-glyph-sidebar-panel-size">
    ...
    >
      <br>
      符文分数根据类型分配。优先给予属于你总符文献祭值最少类型的符文。
      <br>
      <br>
    ...
    >
      <br>
      符文分数根据当前炼金资源总量分配。优先给予关联炼金资源总量最低的符文类型。
      <br>
      <br>
      此模式从不保留符文。
    </div>
    ...
    >
      <br>
      符文将根据<i>当前</i>精炼价值分配价值，并考虑类型特定的资源上限。优先给予价值最高炼金资源的符文；会导致你达到上限的符文实际上价值更低。
      <br>
      <br>
      此模式从不保留符文。
    </div>
