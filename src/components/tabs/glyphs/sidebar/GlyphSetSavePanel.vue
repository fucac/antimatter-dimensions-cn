...
export default {
  ...
  computed: {
    questionmarkTooltip() {
      return `符文预设的工作方式类似时间研究方案，允许你装备一整套之前保存的符文`;
    },
    noSet() {
      return `此槽位未保存符文预设`;
    },
...
<template>
  <div class="l-glyph-sacrifice-options c-glyph-sacrifice-options l-glyph-sidebar-panel-size">
    ...
    <div class="l-glyph-set-save__header">
      加载预设时，尝试匹配以下属性。"精确"只会装备与预设中完全相同的符文。其他设置则大致上允许用"更好"的符文替代装备。
    </div>
    ...
    >
      ...
      <div class="c-glyph-single-set-save-flexbox">
        <div ach-tooltip="设置自定义名称（最多20个字符）">
          <input
            :id="id"
            type="text"
            size="20"
