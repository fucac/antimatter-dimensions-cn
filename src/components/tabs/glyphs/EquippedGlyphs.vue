...
export default {
  ...
  computed: {
    ...
    respecTooltip() {
      ...
      return this.respec
        ? `重置已启用,将于${reset}后将你当前装备的符文放入背包。`
        : `你当前装备的符文将在${reset}后保持装备状态。`;
    },
    undoTooltip() {
      if (!this.undoSlotsAvailable) return "你没有足够的背包空间来卸下符文";
      return this.undoAvailable
        ? ("卸下最后装备的符文,并将现实回溯至你装备它之时。" +
          "(大多数资源将被完全重置)")
        : "撤销仅可用于本次现实期间装备的符文";
    },
    unequipText() {
