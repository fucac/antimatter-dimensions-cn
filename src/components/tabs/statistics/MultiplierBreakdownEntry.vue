...
export default {
  ...
  computed: {
    ...
    disabledText() {
      if (!this.resource.isBase) return `总效果未激活、被禁用或降至 ${formatX(1)}`;
      return Decimal.eq(this.resource.mult, 0)
        ? `你无法获得此资源（未达到转生要求）`
        : `此资源无倍数（转生时将获得 ${format(1)}）`;
    },
  ...
  methods: {
    ...
    dilationString() {
      ...
      const formatFn = resource.isBase
        ? x => format(x, 2, 2)
        : x => formatX(x, 2, 2);
      return `膨胀时间效果：指数${formatPow(this.dilationExponent, 2, 3)}
...
<template>
  <div :class="containerClass">
    ...
    <div class="c-info-list">
      ...
      >
        <div>
          "基础反物质维度产出"是指在你当前反物质维度升级下，假设你在一次献祭后等待一段固定时间
          （根据你的反物质维度数量为 {{ formatInt(10) }}-{{ formatInt(40) }} 秒）所能产出的反物质。
          若你的反物质维度已产出一段时间，此数值可能无法准确反映你的实际产出，但随着游戏推进、数值增大，
          相对偏差会逐渐缩小。
        </div>
        <div v-if="inNC12">
          在普通挑战 12 中，此标签页的明细对某些条目可能不准确，并可能计入作用于所有反物质维度的
          额外倍数，而非仅计入所显示的维度。
        </div>
      </div>
    </div>
