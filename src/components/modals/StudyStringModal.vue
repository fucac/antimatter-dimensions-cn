...
export default {
  ...
  computed: {
    ...
    invalidMessage() {
      ...
      return `你的导入字符串含有无效的研究 ID：${coloredString.replaceAll("#", "").replaceAll(",", ", ")}
        <br><br>`;
    },
    truncatedInput() {
  ...
  methods: {
    ...
    savePreset() {
      if (this.inputIsValid) {
        ...
        GameUI.notify.eternity(`研究树「${this.name}」编辑成功。`);
        this.emitClose();
      }
    },
    deletePreset() {
...
<template>
  ...
  >
      ...
      <div class="c-study-info l-modal-import-tree__tree-info">
        ...
        <template v-else-if="inputIsValidTree">
          ...
          <StudyTreeInfo
            ...
            header-text="以<b>无研究</b>状态加载后的情形："
            :tree-status="importedTree"
          />
          <StudyTreeInfo
    ...
    <div v-if="!isImporting && inputIsValidTree">
      ...
      <PrimaryButton
        v-if="!deleting"
        v-tooltip="'此操作将格式化研究预设文本，例如将 \'a,b,c|d\' 改为 \'a, b, c | d\'。'"
    ...
    <span v-if="isImporting">
      <br>
      <div
        v-tooltip="canEternity ? '' : '你当前无法进行永恒，因此只会执行普通加载。'"
