...
<template>
  ...
  >
    ...
    这将在列表末尾创建一个新的自动执行脚本。
    <span v-if="isImportingExtraData">这还将导入与脚本相关的额外数据。</span>
    <input
      ref="input"
      v-model="input"
      type="text"
    ...
    <div v-if="isValid">
      ...
      <div v-if="hasPresets">
        ...
        <div
          ...
          class="l-has-errors"
        >
          你现有的 {{ formatInt(overwrittenPresetCount) }} 个预设将被导入预设覆盖！
        </div>
        <br>
        <button
          class="o-primary-btn"
      ...
      <div v-if="hasConstants">
        ...
        <div
          ...
          class="l-has-errors"
        >
          <span v-if="willOverwriteConstant">有些现有常量将被覆盖！</span>
          <br v-if="willOverwriteConstant && extraConstants > 0">
          <span v-if="extraConstants > 0">
            由于常量上限为 {{ maxConstantCount }}，将有 {{ formatInt(extraConstants) }} 个常量无法导入。
          </span>
        </div>
        <br>
        <button
