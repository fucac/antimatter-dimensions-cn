...
const GLYPH_NAMES = {
  ...
  music: {
    adjective: { high: "旋律", mid: "和声", low: "调律" },
    // This noun is only used in the case of a single companion reskinned as music (resulting in "亲昵 音乐");
    // otherwise the set's noun will always come from an actual glyph type instead of music
    noun: "音乐"
  },
  effarig: {
    adjective: { both: "元", glyph: "稳固", rm: "机械", none: "碎裂" },
    noun: { both: "鹿颈长", glyph: "稳固", rm: "机理", none: "碎裂" }
  },
  cursed: {
    adjective: { high: "诅咒", mid: "厄咒", low: "凶兆" },
    noun: "诅咒"
  },
  power: {
    adjective: { high: "强能", mid: "精通", low: "潜势" },
    noun: "力量"
  },
  infinity: {
    adjective: { high: "无尽", mid: "无涯", low: "浩瀚" },
    noun: "无限"
  },
  replication: {
    adjective: { high: "繁衍", mid: "模拟", low: "倍增" },
    noun: "复制"
  },
  time: {
    adjective: { high: "时序", mid: "历纪", low: "瞬逝" },
    noun: "时间"
  },
  dilation: {
    adjective: { high: "膨胀", mid: "衰减", low: "稀薄" },
    noun: "膨胀时间"
  },
};
    ...
    basicTypePhrase() {
      ...
      switch (basicGlyphList.length) {
        ...
        case 3:
          ...
          if (basicGlyphList[0].perc === basicGlyphList[2].perc) return "混合异序";
        ...
        case 4:
          ...
          if (basicGlyphList[0].perc === basicGlyphList[1].perc) return "异序杂糅";
          return `${this.getAdjective(basicGlyphList[0])} 杂糅`;
        case 5:
          // This is in reference to the achievement name, and can only occur with exactly one of every basic glyph.
          // Due to music glyphs doubling-up contributions, this may result in a "旋律 皇家同花顺" or similar
          return "皇家同花顺";
        default:
